...
-- departments 테이블에는 있는데 employees 테이블에는 없는 데이터를 같이 출력에줘
SELECT *
  FROM employees e
...
-- Oracle 문법
-- (+)있는 곳에 만족하지 않는 조건도 더 나오게 한다는 뜻
-- LEFT OUTER JOIN
SELECT *
  FROM employees e, departments d
 WHERE e.department_id = d.department_id(+)

-- INNER JOIN은 INNER를 생략 가능하다
-- OUTER JOIN에는 LEFT, RIGHT가 존재하므로 OUTER 생략 가능
