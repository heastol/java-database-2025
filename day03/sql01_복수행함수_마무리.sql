...
-- ROW_NUMBER() - 행번호
-- 전체 employee에서 급여가 높은 사람부터 순위매길 때
...
-- 부서별(departmen_id), 급여 높은 사람부터 랭킹을 매길 때
SELECT first_name || ' ' || last_name AS "full_name"
	 , salary
	 , department_id
	 , rank() OVER (PARTITION BY department_id ORDER BY salary DESC) AS "RANK"
	 , dense_rank() OVER (PARTITION BY department_id ORDER BY salary DESC) AS "DENSE_RANK" -- 일상에서 DENSE_RANK 더 많이 사용
	 , row_number() OVER (PARTITION BY department_id ORDER BY salary DESC) AS "ROW_NUMBER" -- PARTITION BY 잘 사용안함
  FROM employees
 WHERE salary IS NOT NULL
