...
-- 문2. Employees 테이블에서 모든 사원의 이름(last_name)과 연봉을 ' 이름: 1 yewar salary = $연봉'형식으로
-- 출력하고, 컬럼명을 1 Year Salary로 변경하시오(107행)
SELECT last_name || ': 1 Year Salary' || to_char(salary * 12, '$9,999,999.99') AS "1 YEAR salary"
  FROM employees;

-- 문3. 부서별로 담당하는 업무를 한 번 씩만 출력하시오(20행)
 ...
 * WHERE절, ORDER BY절
 */
-- 문4. employyes에서 급여가 7000~10000달러 범위이외 사람의 이름과 성을 full_name, 급여를 오름차순으로 출력하시오(75행)
SELECT *
  FROM employees
 WHERE salary < 7000
    OR salary> 10000;
...
-- 입사일자가 2005년 5월 20일부터 2006년 5월 20일 사이에 입사한 사원의 이름(full_nsme), 사원번호, 고용일자를 출력하시오.
-- 단 입사일을 빠르순으로 정렬하시오.
...
SELECT first_name || last_name AS "full_name"
	 ...
	 , to_char(hire_date, 'yyyy-mm-dd') AS "hire_date"
  FROM employees
 WHERE hire_date BETWEEN '2005-05-20' AND '2006-05-20'
...
SELECT employee_id
	 , first_name || last_name AS "full_name"
	 , salary
	 , salary + (salary * 0.153) AS "Increased_Salary(참조)"
 ...
 * JOIN, 
 */
--문9. employees, department, locations 테이블 구조 파악
-- Oxford에 근무하는 사원의 full_name, 업무, 무서명, 도시명을 출력히사오
 ...
 * 서브쿼리
 */
-- 문11. Tucker 사우너보다 급여를 많이 받는 사원의 full_name,업무, 급여를 출력(15행)
SELECT *
  FROM employees
